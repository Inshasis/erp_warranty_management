# Copyright (c) 2022, Matiyas Solutions LLP and contributors
...
class DistributorSerialNoCannotCreateDirectError(ValidationError):
	pass


class DistributorSerialNoCannotCannotChangeError(ValidationError):
	pass


class DistributorSerialNoNotRequiredError(ValidationError):
	pass


class DistributorSerialNoRequiredError(ValidationError):
	pass


class DistributorSerialNoQtyError(ValidationError):
	pass


class DistributorSerialNoItemError(ValidationError):
	pass


class DistributorSerialNoWarehouseError(ValidationError):
	pass


class DistributorSerialNoBatchError(ValidationError):
	pass


class DistributorSerialNoNotExistsError(ValidationError):
	pass


class DistributorSerialNoDuplicateError(ValidationError):
	pass


class DistributorSerialNo(StockController):

	def __init__(self, *args, **kwargs):
		super(DistributorSerialNo, self).__init__(*args, **kwargs)
	...
	def validate(self):
		if self.get("__islocal") and self.warehouse and not self.via_stock_ledger:
			frappe.throw(
				_(
					"New Distributor Serial No cannot have Warehouse. Warehouse must be set by Stock Entry or Purchase Receipt"
				),
				DistributorSerialNoCannotCreateDirectError,
			)
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = frappe.db.get_value(
				"Distributor Serial No", self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				frappe.throw(
					_("Item Code cannot be changed for Distributor Serial No."), DistributorSerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				frappe.throw(
					_("Warehouse cannot be changed for Distributor Serial No."), DistributorSerialNoCannotCannotChangeError)

	def validate_item(self):
		"""
		Validate whether Distributor Serial No is required for this item
		"""
		item = frappe.get_cached_doc("Item", self.item_code)
		if item.has_serial_no != 1:
			frappe.throw(
				_("Item {0} is not setup for Distributor Serial Nos. Check Item master").format(
					self.item_code)
			)
		...
		self.warranty_period = item.distributor_warranty_period
		...
		if purchase_sle:
			...
			if purchase_sle.voucher_type in ("Purchase Receipt", "Purchase Invoice"):
				self.supplier, self.supplier_name = frappe.db.get_value(
					purchase_sle.voucher_type, purchase_sle.voucher_no, [
		...
		if delivery_sle:
			...
			if delivery_sle.voucher_type in ("Delivery Note", "Sales Invoice"):
				self.customer, self.customer_name = frappe.db.get_value(
					delivery_sle.voucher_type, delivery_sle.voucher_no, [
						"customer", "customer_name"]
				)
			if self.warranty_period:
				self.warranty_expiry_date = add_days(
					cstr(delivery_sle.posting_date), cint(
	...
	def on_trash(self):
		...
		if sle_exists:
			frappe.throw(
				_("Cannot delete Distributor Serial No {0}, as it is used in stock transactions").format(
					self.name)
			)

	def before_rename(self, old, new, merge=False):
		if merge:
			frappe.throw(_("Sorry, Distributor Serial Nos cannot be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		for dt in frappe.db.sql(
			...
			for item in frappe.db.sql(
				...
				serial_nos = map(lambda i: new if i.upper()
									== old.upper() else i, item[1].split("\n"))
				frappe.db.sql(
...
def validate_serial_no(sle, item_det):
	...
	if item_det.has_serial_no == 0:
		if serial_nos:
			frappe.throw(
				_("Item {0} is not setup for Distributor Serial Nos. Column must be blank").format(sle.item_code),
				DistributorSerialNoNotRequiredError,
			)
	elif not sle.is_cancelled:
		if serial_nos:
			if cint(sle.actual_qty) != flt(sle.actual_qty):
				frappe.throw(
					_("Distributor Serial No {0} quantity {1} cannot be a fraction").format(sle.item_code, sle.actual_qty)
				)
			...
			if len(serial_nos) and len(serial_nos) != abs(cint(sle.actual_qty)):
				frappe.throw(
					...
					DistributorSerialNoQtyError,
				)

			if len(serial_nos) != len(set(serial_nos)):
				frappe.throw(
					_("Duplicate Distributor Serial No entered for Item {0}").format(sle.item_code), DistributorSerialNoDuplicateError
				)

			for serial_no in serial_nos:
				if frappe.db.exists("Distributor Serial No", serial_no):
					...
					if sr.item_code != sle.item_code:
						if not allow_serial_nos_with_different_item(serial_no, sle):
							frappe.throw(
								_("Distributor Serial No {0} does not belong to Item {1}").format(serial_no, sle.item_code),
								DistributorSerialNoItemError,
							)
					...
					if cint(sle.actual_qty) > 0 and has_serial_no_exists(sr, sle):
						...
						frappe.throw(
							_("Distributor Serial No {0} has already been received in the {1} #{2}").format(
								frappe.bold(serial_no), sr.purchase_document_type, doc_name
							),
							DistributorSerialNoDuplicateError,
						)
					...
					if (
						...
						if return_against and return_against != sr.delivery_document_no:
							frappe.throw(_("Distributor Serial No {0} has been already returned").format(sr.name))

					if cint(sle.actual_qty) < 0:
						if sr.warehouse != sle.warehouse:
							frappe.throw(
								_("Distributor Serial No {0} does not belong to Warehouse {1}").format(serial_no, sle.warehouse),
								DistributorSerialNoWarehouseError,
							)

						if not sr.purchase_document_no:
							frappe.throw(_("Distributor Serial No {0} not in stock").format(serial_no), DistributorSerialNoNotExistsError)

						if sle.voucher_type in ("Delivery Note", "Sales Invoice"):

							if sr.batch_no and sr.batch_no != sle.batch_no:
								frappe.throw(
									_("Distributor Serial No {0} does not belong to Batch {1}").format(serial_no, sle.batch_no),
									DistributorSerialNoBatchError,
								)

							if not sle.is_cancelled and not sr.warehouse:
								frappe.throw(
									_("Distributor Serial No {0} does not belong to any Warehouse").format(serial_no),
									DistributorSerialNoWarehouseError,
								)

							# if Sales Order reference in Distributor Serial No validate the Delivery Note or Invoice is against the same
							if sr.sales_order:
								if sle.voucher_type == "Sales Invoice":
									if not frappe.db.exists(
										...
										frappe.throw(
											_(
												"Cannot deliver Distributor Serial No {0} of item {1} as it is reserved to fullfill Sales Order {2}"
			...
			frappe.throw(
				_("Distributor Serial Nos Required for Serialized Item {0}").format(sle.item_code), DistributorSerialNoRequiredError
			)
	elif serial_nos:
		# SLE is being cancelled and has Distributor Serial Nos
		for serial_no in serial_nos:
...
def check_serial_no_validity_on_cancel(serial_no, sle):
	sr = frappe.db.get_value(
		"Distributor Serial No", serial_no, ["name", "warehouse", "company", "status"], as_dict=1
	)
	sr_link = frappe.utils.get_link_to_form("Distributor Serial No", serial_no)
	...
	if sr and (actual_qty < 0 or is_stock_reco) and (sr.warehouse and sr.warehouse != sle.warehouse):
		...
		msg = _("Cannot cancel {0} {1} as Distributor Serial No {2} does not belong to the warehouse {3}").format(
		...
		if sr.warehouse:
			# Distributor Serial No is active in another warehouse/company.
			msg = _("Cannot cancel {0} {1} as Distributor Serial No {2} is active in warehouse {3}").format(
				sle.voucher_type, doc_link, sr_link, frappe.bold(sr.warehouse)
			)
		elif sr.company != sle.company and sr.status == "Delivered":
			# Distributor Serial No is inactive (allowed) or delivered from another company (block).
			msg = _("Cannot cancel {0} {1} as Distributor Serial No {2} does not belong to the company {3}").format(
...
def validate_so_serial_no(sr, sales_order):
	if not sr.sales_order or sr.sales_order != sales_order:
		...
		frappe.throw(_("""{0} Distributor Serial No {1} cannot be delivered""").format(msg, sr.name))
...
def allow_serial_nos_with_different_item(sle_serial_no, sle):
	"""
	Allows same Distributor Serial Nos for raw materials and finished goods
...
def get_new_serial_number(series):
	sr_no = make_autoname(series, "Distributor Serial No")
	if frappe.db.exists("Distributor Serial No", sr_no):
...
def auto_make_serial_nos(args):
	...
	for serial_no in serial_nos:
		is_new = False
		if frappe.db.exists("Distributor Serial No", serial_no):
			sr = frappe.get_cached_doc("Distributor Serial No", serial_no)
		elif args.get("actual_qty", 0) > 0:
			sr = frappe.new_doc("Distributor Serial No")
			is_new = True
	...
	form_links = list(map(lambda d: get_link_to_form("Distributor Serial No", d), created_numbers))
	...
	if len(form_links) == 1:
		frappe.msgprint(_("Distributor Serial No {0} Created").format(form_links[0]), singular_title)
...
def update_serial_no_after_submit(controller, parentfield):
...
def update_maintenance_status():
	serial_nos = frappe.db.sql(
		"""select name from `tabDistributor Serial No` where (amc_expiry_date<%s or
		warranty_expiry_date<%s) and maintenance_status not in ('Out of Warranty', 'Out of AMC')""",
		(nowdate(), nowdate()),
	)
	for serial_no in serial_nos:
		doc = frappe.get_doc("Distributor Serial No", serial_no[0])
		doc.set_maintenance_status()
		frappe.db.set_value("Distributor Serial No", doc.name, "maintenance_status", doc.maintenance_status)


def get_delivery_note_serial_no(item_code, qty, delivery_note):
	serial_nos = ""
	dn_serial_nos = frappe.db.sql_list(
		""" select name from `tabDistributor Serial No`
...
@frappe.whitelist()
def auto_fetch_serial_number_dis(
	qty: float,
	item_code: str,
	warehouse: str,
...
def get_delivered_serial_nos(serial_nos):
	...
	SerialNo = frappe.qb.DocType("Distributor Serial No")
